...
//                        Class arrayItemType = [[self class] performSelector:NSSelectorFromString([NSString stringWithFormat:@"%@_class", key])];
                        Class arrayItemType = [self  performSelector:NSSelectorFromString([NSString stringWithFormat:@"%@_item_class", key])];
            ...
            // Guava 2017.9.21 fix: in iOS 8.3, set "false" to a BOOL property will crash.
            if(@available(iOS 8.0, *)) {
                ...
                if([value isKindOfClass:[NSString class]] && [attribute hasPrefix:@"Tc"]) {
...
- (NSString *)description {
    ...
    return [NSString stringWithFormat:@"#<%@:%@ id = %@ %@>", [self class],[super description], self.objectId, [dic description]];
}
